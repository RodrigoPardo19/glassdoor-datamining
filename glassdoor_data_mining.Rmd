...
```{r}
# a<-glassdoor_data$header.jobTitle
# dtm.sw <- DocumentTermMatrix(a)
```
...
Se pueden crear modelos que midan la tasa de cambio o incorporación de las nuevas habilidades que les van a solicitando a los postulantes. Es sabido que en 
...
#Salarios: salary.salaries
#Rating:  rating.starRating  

# Muestra los primeros 8 datos y las columnas seleccionadas
tablaEmpresa[0:8, c("salary.salaries","rating.starRating")]


# si mientras mas sueldos pagan mas rating tienen

# beneficios y rating


# diferencia si la empresa tienen mejor rating por tener mejores beneficios o por el sueldo.
...
#4. Debe incluir un enlace al código fuente. El código fuente debe estar en un repositorio (GitLab o GitHub) donde todos los miembros del equipo participen. El repositorio debe estar visible para la evaluación. El código fuente utilizado debe mostrar todo lo utilizado generar sus estadísticas y análisis (e.g. por ej. generar la página usando jupyter notebook, o markdown R, o poner enlaces a sus scripts. Mientras más reproducible el trabajo, mejor. El código fuente no se cuenta dentro del largo de las 5 páginas). La calidad del código fuente será evaluada en términos de que permita replicar el estudio.
